...
注意：task 5.1 会将 vocab.py 更新为新的 特征名称。
...
def _is_feature_names_updated():
    """
    检查 FEATURE_NAMES 是否已更新为 特征集（task 5.1 完成后）。
...
class TestConfigInputDimSmoke:
    def test_input_dim_equals_65(self):
        """Config.INPUT_DIM must equal the registry feature count (65)."""
...
class TestFeatureNamesSmoke:
    ...
    def test_liq_score_not_in_feature_names(self):
        ...
        if not _is_feature_names_updated():
            pytest.skip(
                "vocab.py still contains old Solana features — "
                "pending task 5.1 (FeatureEngineer implementation)"
            )
    ...
    def test_feature_names_length_matches_input_dim(self):
        """FEATURE_NAMES 的长度应等于 Config.INPUT_DIM（65）"""
        feature_names = _get_feature_names()
        if feature_names is None:
            pytest.skip("model_core.vocab.FEATURE_NAMES not available yet (pending task 5.1)")
        if not _is_feature_names_updated():

...
def test_record_and_summarize_session(isolated_training_time: Path) -> None:
    ...
    # live 会话的 started_at 必须相对“现在”，否则随真实时钟流逝而增长
    # （历史日期固定在 2026-07-14 会导致 session_seconds ≈ 数月）。
    job_start = datetime.now(timezone.utc) - timedelta(minutes=5)
    job = {
        "symbol": "XAUUSD",
    ...
    assert live.session_seconds == pytest.approx(300, abs=10)

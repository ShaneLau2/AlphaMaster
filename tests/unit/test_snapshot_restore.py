...
class TestSnapshotRestoreAttentionParams:
    ...
    def test_qk_projection_params_match_snapshot_after_restore(self, model_with_snapshot):
        """恢复快照后，所有含 'in_proj' 的 QKV 投影参数应与快照逐元素相等。

        当前 AlphaGPT 结构（registry 时代）不再有独立 'qk_norm' 参数，
        attention 入口为 in_proj_weight/in_proj_bias（QKV 合并投影）。
        """
        model, snapshot = model_with_snapshot
        _apply_snapshot_restore_with_ffn_noise(model, snapshot)
        ...
        for name, snap_tensor in snapshot.items():
            if 'in_proj' in name:
                ...
                checked += 1
        assert checked > 0, "未找到含 'in_proj' 的参数，请检查模型结构"

...
class TestComputeFeaturesShape:
    """compute_features 输出形状应为 [N, 65, T]（registry 特征数，需求 F1.1, F1.2）"""

    def test_output_shape_default(self):
        raw = _make_raw_dict(N=3, T=50)
        out = FeatureEngineer.compute_features(raw)
        assert out.shape == (3, 65, 50), (
            f"Expected shape (3, 65, 50), got {tuple(out.shape)}"
        )
    ...
    def test_feature_dim_equals_65(self):
        """feature 维度为 65，对应 FEATURE_REGISTRY 特征数（需求 F1.7）"""
        raw = _make_raw_dict(N=3, T=50)
        out = FeatureEngineer.compute_features(raw)
...
class TestRet20PrefixZero:
    ...
    def test_ret20_raw_first20_are_zero(self):
        raw = _make_raw_dict(N=3, T=50)
        close  = raw["close"].float()
        ret20_raw = FeatureEngineer._ret20(close)
        prefix = ret20_raw[:, :20]

...
关键数字（当前 registry 派生）：
  - 特征数  F = 65（8 大类）
  - 算子数  O = 62（GT/LT/AND/OR 纯 0/1 二值算子已移除，见 ops.py P1 注）
  - vocab size  = F + O = 127
  - feat_offset = 65
...
# ── 被测模块 ─────────────────────────────────────────────────────────────
from model_core.features import FeatureEngineer, FEATURE_NAMES
...
class TestKeyNumbers:
    ...
    def test_operator_count(self):
        """算子数应 == 62（44 初始 + 3 CS + 8 Task3.3 + 7 Task3.4；
        GT/LT/AND/OR 二值算子已从词表移除）。"""
    ...
    def test_feat_offset(self):
        """feat_offset == 65（feature token id ∈ [0,64]，operator id ∈ [65,126]）。"""
...
class TestStackVM:
    ...
    def test_op_map_size_matches_ops_config(self):
        """vm.op_map 应覆盖全部算子 token（数量 = len(OPS_CONFIG) = 62）。"""
        vm = StackVM()
...
class TestEvaluatorPipeline:
    ...
    def setup(self):
        ...
        feats = FeatureEngineer.compute_features(raw)   # [3, 65, 200]
        vm = StackVM()

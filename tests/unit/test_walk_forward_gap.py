...
class TestNormalCase:
    ...
    def test_train_start_advances_rolling_window(self):
        """rolling window：每折 train_start = (k-1) * fold_size。

        engine 已从 expanding（train_start 恒 0）改为 rolling，避免早期折的
        val 切片被后续折的 train 包含导致 val_score 不再严格 OOS。
        """
        T, n_folds, gap = 500, 5, 20
        fold_size = T // n_folds  # 100
        folds = _build_walk_forward_folds(T=T, n_folds=n_folds, gap=gap)
        assert len(folds) > 0
        expected = [i * fold_size for i in range(len(folds))]
        for i, fold in enumerate(folds):
            assert fold["train_start"] == expected[i], (
                f"折 {i}: train_start 应为 {expected[i]}（rolling），"
                f"实际为 {fold['train_start']}"
            )

"""
Property-based tests for model_core.features (FeatureEngineer).
...
@settings(max_examples=50, deadline=None)  # 本机高负载下单例可超 200ms 默认 deadline（假阳性）
def test_compute_features_shape_and_nan_free(N: int, T: int):
    """
    Property 1: compute_features Output Shape and NaN Safety Invariant

    For any N ∈ [1, 10] and T ∈ [21, 200] with positive OHLCV inputs,
    compute_features must return a tensor of shape [N, 65, T] with no NaN/Inf.
    (65 = FEATURE_REGISTRY 特征数)

    **Validates: Requirements F1.1, F1.2, F1.10**
    """
    raw_dict = _make_ohlcv(N, T)
    out = FeatureEngineer.compute_features(raw_dict)

    # Shape invariant: 65 registry features
    assert out.shape == (N, 65, T), (
        f"Expected shape ({N}, 65, {T}), got {tuple(out.shape)}"
    )
...
@settings(max_examples=50, deadline=None)  # 高负载机器 200ms 默认 deadline 假阳性
...
def test_atr_non_negative(N: int, T: int) -> None:
    ...
    FeatureEngineer._atr raw output (before log1p) must satisfy
    all values >= 0.
    ...
    **Validates: Requirements F1.3**
    ...
    atr_raw = FeatureEngineer._atr(close, high, low)
...
def test_rvol_positive(N: int, T: int) -> None:
    ...
    FeatureEngineer._rvol output must satisfy all values >= 1e-9
    ...
    **Validates: Requirements F1.4**
    ...
    rvol_raw = FeatureEngineer._rvol(close)
...
def test_ret20_prefix_zero(N: int, T: int) -> None:
    ...
    FeatureEngineer._ret20 output must be exactly 0, and positions
    ...
    **Validates: Requirements F1.5**
    ...
    ret20 = FeatureEngineer._ret20(close)

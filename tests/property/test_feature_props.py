# Feature: alphagpt-refactor, Property 3: 特征张量形状不变量
# Feature: alphagpt-refactor, Property 4: 特征值有界性
"""
...
@settings(max_examples=100, deadline=None)  # 高负载机器 200ms 默认 deadline 假阳性
@given(
...
def test_property3_feature_tensor_shape(n: int, t: int):
    """
    For any N symbols and T timesteps, FeatureEngineer.compute_features()
    must return a tensor of shape exactly [N, 65, T].

    Feature count is registry-driven: 65 features (FEATURE_NAMES).
    ...
    assert features.shape == (n, 65, t), (
        f"Expected shape ({n}, 65, {t}), got {tuple(features.shape)}"
    )
...
def test_property4_feature_values_bounded_no_nan_inf(raw_dict: dict):
    ...
    output of FeatureEngineer.compute_features() must:

...
def test_alphagpt_forward_any_length(seq_len):
    ...
    **Validates: Requirements T5.2, T5.3, T5.4, T5.5**
    ...
    # P2-1 修复后 value 恒为 None（critic head 已移除，保持 3 元组签名兼容）
    if value is not None:
        assert not torch.isnan(value).any(), (
            f"value contains NaN for seq_len={seq_len}"
        )
    # P2-1 修复后 task_probs 亦可能为 None（MTPHead 简化为单 head）
...
def test_ic_arithmetic_consistency(N: int, T: int) -> None:
    ...
    （P2 对齐修正：factor[t] 在 open[t+1] 成交、持到 open[t+2]，收益即
    target_ret[t]，故配对 factor[t]~target_ret[t]；尾部两根边界 target 恒 0 裁剪。）

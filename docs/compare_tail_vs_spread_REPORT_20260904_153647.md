...
|---|---|---|---|---|---|---|---|---|
| tail（最近 N 根） | 42 | 2.1634 | -0.3691 | ✗ 未过 | -6.42 | -0.171 | -0.67% | 3.3004 |
| spread（全历史分层分块） | 7 | 0.8317 | -0.2971 | ✗ 未过 | -8.80 | -0.357 | -1.18% | 1.3324 |

> 代表口径：**跨 seed 中位 holdout** —— 取各 seed holdout val_score 处于中位数位置的那次 run（偶数个时取离中位数最近、并列取低者）。不用 in-sample best 选代表，避免挑中样本外彩票 seed；“跨seed中位 holdout”列 = 该变体全部 seed holdout 分的中位数。
...
- 两路代表均未通过 holdout 闸门（短训期属正常）：对比仅作相对参考，建议加大步数后再判断。
- 相对参考：tail 代表样本外分 -0.3691 vs spread -0.2971（选高者，注意未过闸门无统计意义）。

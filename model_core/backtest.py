"""
model_core/backtest.py — 连续撮合回测评估器（组合级多目标 Reward）
...
class ContinuousBacktest:
    ...
    def _ts_ic_stability(self, factors: Tensor, target_ret: Tensor) -> float:
        ...
        P2 对齐修正：target_ret[t] = log(open[t+2]/open[t+1])，position[t]
        （=tanh(factor[t])）产生 open[t+1]→open[t+2] 的收益，故 IC 必须配对
        factor[t]~target_ret[t]（原实现错配 factor[t]~target_ret[t+1]，比 PnL
        晚一根 bar）。尾部两根边界 target 恒 0，退出配对。

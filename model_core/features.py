"""
model_core/features.py -- Feature Engineering (20 features)
...
class FeatureEngineer:
    """Feature Engineer (30 features, v3.0)."""
    ...
    def _atr(close: torch.Tensor, high: torch.Tensor,
        ...
        return FeatureEngineer._rolling_mean(tr, w)

    @staticmethod
    def _rvol(close: torch.Tensor, w: int = 20) -> torch.Tensor:
        eps = FeatureEngineer._EPS
    ...
    def _rsi(close: torch.Tensor, w: int = 14) -> torch.Tensor:
        ...
        avg_g  = FeatureEngineer._rolling_mean(gains,  w)
        avg_l  = FeatureEngineer._rolling_mean(losses, w)
    ...
    def _boll_pos(close: torch.Tensor, w: int = 20) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        eps = FeatureEngineer._EPS
        ma = FeatureEngineer._rolling_mean(close, w)
        std = FeatureEngineer._rolling_std(close, w)
        upper = ma + 2 * std
        lower = ma - 2 * std
    ...
    def _macd_hist(close: torch.Tensor) -> torch.Tensor:
        ...
        macd = FeatureEngineer._ema_simple(close, 12) - FeatureEngineer._ema_simple(close, 26)
        signal = FeatureEngineer._ema_simple(macd, 9)
        return macd - signal
    ...
    def _obv_slope(close: torch.Tensor, volume: torch.Tensor, w: int = 20) -> torch.Tensor:
        ...
        # OBV 斜率（用线性回归）
        return FeatureEngineer._linear_slope(obv, w)
    ...
    def _mfi(close: torch.Tensor, high: torch.Tensor,
        ...
        pos_sum = FeatureEngineer._rolling_mean(pos_mf, w) * w
        neg_sum = FeatureEngineer._rolling_mean(neg_mf, w) * w
    ...
    def _cci(close: torch.Tensor, high: torch.Tensor,
        ...
        eps = FeatureEngineer._EPS
        typical = (high + low + close) / 3.0
        ma = FeatureEngineer._rolling_mean(typical, w)
...
FeatureEngineer.INPUT_DIM = len(FEATURE_REGISTRY.feature_names)

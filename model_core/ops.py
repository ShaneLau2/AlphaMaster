...
def _ema_simple(x: torch.Tensor, span: int, exact: bool = False) -> torch.Tensor:
    ...
    语义（与历史完全一致）：out[0] = x[0]；out[t] = alpha·x[t] + (1-alpha)·out[t-1]，
    alpha = 2/(span+1)。

    R-vectorized（取代 Python 逐 t 递推，E2 后续优化）：EMA 是一阶线性递推，
    闭式解 out[t] = a^t·x[0] + alpha·Σ_{j=1..t} a^{t-j}·x[j]（a = 1-alpha），
    即「截断因果卷积 + 种子项精确补偿」。用 F.conv1d 一次性求全部时间步：
    核长 L 取使 a^L ≤ 1e-8（span=5→46，span=20→185），截断残差 ≤ a^L·max|x|
    ≤ 1e-8·max|x|，远低于 float32 递推自身的累积舍入（~1e-6 级）——即在浮点
    噪声级内与原递推等价（实测 max|Δ| ~ 5e-7，|x|≤5），不再是 P1-10 曾拒绝的
    ~1e-4 级近似。长序列（T > 2L+2；训练与实盘均满足）走同一条 conv 路径，
    无 train-serve 路径分叉；短序列 / 含非有限值时退回原 exact 递推
    （与旧行为逐位一致）。

    实测（E2 后续）：N=1, T=12000 下 conv 路径 ~0.5–1.2ms（原递推 ~200ms）；
    [B=31, T] 批量下 EMA_20 ~9x——瓶颈从 Python 逐 t 调度转移为 BLAS 卷积。

    参数：
        span: EMA 周期
        exact: 保留参数兼容性（本实现已统一路径；传 True 亦不再强制旧递推）
    """
    alpha = 2.0 / (span + 1.0)
    N, T = x.shape
    ...
    # ── 向量化路径：截断因果卷积（残差 ≤ a^L·max|x| ≤ 1e-8·max|x|）────
    # 核长 L：a^L ≤ 1e-8；短序列（T ≤ 2L+2）conv 零填充占比过高、舍入路径
    # 分叉收益低，退回 exact 递推保证与旧行为逐位一致。
    L = min(T, int(math.ceil(math.log(1e-8) / math.log(max(a, 1e-12)))))
    if T > 2 * L + 2 and torch.isfinite(x).all():
        try:
            # conv1d 核 w[k] = alpha·a^(L-1-k)，k=0..L-1（最旧→最新）。
            # 左补 L-1 个零后交叉相关：conv[t] = alpha·Σ_{j=max(0,t-L+1)..t}
            # a^(t-j)·x[j]（窗口内权重精确）。
            kk = torch.arange(L, dtype=x.dtype, device=x.device)
            w = (alpha * torch.pow(a, (L - 1 - kk).float())).view(1, 1, L)
            pad = torch.zeros(N, L - 1, device=x.device, dtype=x.dtype)
            xp = torch.cat([pad, x], dim=1)                  # [N, T+L-1]
            conv = torch.nn.functional.conv1d(
                xp.unsqueeze(1), w).squeeze(1)               # [N, T]
            # 种子项精确补偿：递推 out[t] = a^t·x[0] + Σ_{j>=1}；conv 把
            # j=0 的权重算成 alpha·a^t（且仅在 t<L 时窗口含 x[0]）。
            # 修正 = a^t·x[0] − alpha·a^t·[t<L]·x[0]。
            tt = torch.arange(T, dtype=x.dtype, device=x.device)
            at = torch.pow(a, tt)                            # [T]，大 t 自然下溢为 0
            corr = x[:, :1] * at * (1.0 - alpha * (tt < L).float())
            return conv + corr
        except Exception:  # noqa: BLE001 卷积路径异常退回 exact 递推
            pass

    # ── exact 递推回退（短序列 / 非有限输入 / conv 异常）──────────────

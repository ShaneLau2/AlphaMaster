...
class StackVM:
    ...
    # ── 批量执行（E2 可行性验证后的生产入口）─────────────────────────────
    # 把同“位置骨架”的公式合并成 [B,T] 一次跑：位置骨架 = 特征槽统一记 -1、
    # 算子保留原 token id（特征 id 可变、算子位置必须一致），保证同组在任意
    # 一步要么全是特征（可各自不同 id）要么全是同一算子 → 安全批量。
    # CS 算子（跨品种；N=1 单品种时逐行退化为恒等/常量语义）在 B>1 时逐行
    # 应用，保持与串行 execute 完全一致。
    ...
    def execute_batched(
        ...
        """批量执行多条公式，返回与输入一一对齐的 [1,T] 结果（逐条同口径）。

        语义保证：对每条公式，输出与串行 ``execute(fml, feat_tensor)``
        逐元素一致（同一算子实现、同一 clean/normalize 路径）。
        仅支持 N=1（单品种 [1,C,T] 输入）——N>1 或形状异常时自动退化为
        逐条执行，保语义不变。

        返回：list，长度 = len(formulas)，元素为 [1,T] 张量或 None（非法/
        异常公式与串行 execute 返回 None 对齐）。
        """
        out: list[torch.Tensor | None] = [None] * len(formulas)
        # 形状/N 维守卫：只有单品种 [1,C,T] 才批量；否则逐条保语义
        if feat_tensor.ndim != 3 or feat_tensor.shape[0] != 1:
            for i, fml in enumerate(formulas):
                out[i] = self.execute(fml, feat_tensor)
            return out
        feat = feat_tensor[0]                # [C, T]
        C = feat.shape[0]

        # 分组 + 预筛非法公式（特征越界 / 未知算子 → 该条 None，不进组）
            ...
            for t in fml:
                if t < self.feat_offset:
                    if t >= C:
                        bad = True
                        break
                elif t not in self.op_map:
                    bad = True
                    break
            if bad:
                continue
            key = tuple(-1 if t < self.feat_offset else t for t in fml)
        ...
        for key, idxs in groups.items():
            ...
            try:
                ...
                for pos, k in enumerate(key):
                    if k == -1:
                        # 特征槽：一次高级索引取 B 行（各公式特征可不同）→ [B, T]
                        toks = torch.tensor(
                            [formulas[i][pos] for i in idxs],
                            dtype=torch.long, device=feat.device)
                        stack.append(feat.index_select(0, toks))   # [B, T]
                    else:
                        arity = self.arity_map.get(k, 1)
                        if len(stack) < arity:
                            ok = False
                            break
                        args = [stack.pop() for _ in range(arity)][::-1]
                        fn = self.op_map[k]
                        if B > 1 and names[k] in self._CS_NAMES:
                            # N=1 单品种 CS 语义：逐行应用保持与串行一致
                            rows = []
                            for b in range(B):
                                a_b = [a[b].unsqueeze(0) for a in args]
                                rows.append(fn(*a_b).squeeze(0))
                            res = torch.stack(rows)
                        else:
                            res = fn(*args)
                        if torch.isnan(res).any() or torch.isinf(res).any():
                ...
                for bi, i in enumerate(idxs):
                    # 逐行标准化（N=1 滚动 zscore），与串行 [1,T] 输出一致
                    out[i] = self._normalize_output(final[bi].unsqueeze(0))
            except Exception:
                # 该组异常：与串行 execute 的 None 对齐（不中断其余组）
                continue
        return out

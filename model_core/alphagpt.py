...
class AlphaGPT(nn.Module):
    def __init__(self):
        ...
        # 【P2-1 修复】：原 head_critic 未使用已移除。默认 critic=None（纯 policy，
        # 行为与历史一致）；实验 ACC_CRITIC_GAE 时经 enable_critic() 挂价值头，
        # forward 返回每行价值供 GAE 优势/批评损失使用。
        self.critic: nn.Linear | None = None

    def enable_critic(self) -> "AlphaGPT":
        """实验（ACC_CRITIC_GAE）：附加 critic 价值头（last_emb → 标量）。

        在 AlphaEngine.__init__ 构造 optimizer 之前调用，确保 critic 参数
        进入同一 AdamW。默认关闭 = 不建头，与历史行为完全一致。
        """
    ...
    def forward(self, idx):
        ...
        # 默认 value=None（纯 policy，签名兼容旧调用方）；
        # ACC_CRITIC_GAE 实验：critic 价值头预测状态价值 [B]（squeeze 掉单维）。
        value = None
        if self.critic is not None:
            value = self.critic(last_emb).squeeze(-1)

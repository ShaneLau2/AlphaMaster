...
class BacktestEngine:
    ...
    def __init__(
        ...
        # 每笔投入上限（占权益 %）：仓位 = tanh(因子) × 上限%。100% = 现状。
        self.max_position_pct = min(200.0, max(1.0, float(max_position_pct or 100.0)))
        # 无信号阈值：|tanh(因子)| < min_abs → 空仓观望。None = Config.MIN_TRADE_EXPOSURE。
        if min_abs is None:
            try:
                from config import Config
                self.min_abs = float(getattr(Config, "MIN_TRADE_EXPOSURE", 0.05))
    ...
    def _backtest_symbol(
        ...
        # 再乘「每笔投入上限」（占权益 %）：上限 5% → 仓位 ∈ [-0.05, +0.05]
        _cap = self.max_position_pct / 100.0
        signal_np   = np.tanh(factor_np) * _cap
        if self.min_abs > 0:
            signal_np[np.abs(np.tanh(factor_np)) < self.min_abs] = 0.0  # 观望区 FLAT

...
class Config:
    ...
    # ── 数据参数 ──────────────────────────────────────────
    # 每品种拉取的历史 K 线上限（本地 Parquet 归档数据不受此上限约束）。
    BARS_COUNT            = 10_000_000
    MIN_BARS              = 300    # 低于此值的品种被排除（最低要求：>200 NORM_WINDOW + WF余量）
    ...
    INPUT_DIM       = 65           # 特征数（与 FeatureEngineer.INPUT_DIM 一致 = len(FEATURE_NAMES) = 65）
    ...
    # ── 文件路径 ──────────────────────────────────────────
    STRATEGY_FILE  = "best_default.json"
    ...
    def get_timeframe(cls, tf_str: str) -> int:
        ...
        Returns:
            对应的 TIMEFRAME 整数常量（沿用终端约定，供历史数据兼容）

...
def data_file_for_web() -> str:
    """返回一个本地 Parquet 作为 Web 端到端测试数据（web_settings 优先）。"""
    settings = json.loads((ROOT / "web_settings.json").read_text(encoding="utf-8"))
    path = settings.get("last_data_file", "")
    if not path or not Path(path).exists():
        cands = sorted((ROOT / "data" / "slices").glob("*.parquet"))
        if not cands:
            cands = sorted((ROOT / "data" / "training").glob("*.parquet"))

...
def build() -> Path:
    ...
    add_p(
        doc,
        'python -c "import torch,fastapi,pyarrow,matplotlib,multipart; print(\'OK\')"',
        bold=True,
    )
    add_tip(
        doc,
    ...
    add_steps(
        doc,
        [
            "选择数据源：例如 TradingView / Binance / OKX / 通达信。",
    ...
    add_h2(doc, "11.6 数据源不可用")
    add_bullets(
        doc,
        [
            "在「实时分析/数据下载」页看下拉里的数据源是否带（不可用）标记，悬停查看原因。",
            "网络类源（TradingView）按弹窗提示开 VPN 或换云服务器；A 股用通达信。",
        ],
    )
    ...
    add_steps(
        ...
        [
            ...
            "过一阵点停止（或等分数抬升后停）。",
            "回测页 → 选择刚生成的策略 → 开始回测。",
            "实时页 → 选一个可用数据源 → 填同一品种与周期 → 选同一策略 → 添加。",
        ],
    )

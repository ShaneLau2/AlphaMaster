...
def download_history(
    ...
    progress_cb=None,
    after_start_ms: int | None = None,
) -> pd.DataFrame:
    """拉历史 K 线（翻页向更早处）。

    after_start_ms（epoch 毫秒）给定时代替「现在」作首页锚点：增量归档重复
    下载时锚定到档案最早 bar 之前，向更早历史扩展。
    """
    rows: list[list[str]] = []
    after: str | None = str(after_start_ms) if after_start_ms is not None else None
    stagnant = 0
    page = 0

...
def compute_target_positions(
    ...
    min_abs 为无信号阈值（|tanh(factor)| 低于该值 → 空仓观望）；
    缺省时读取 Config.MIN_TRADE_EXPOSURE（训练侧默认 0.05）。

    Args:
        factors:        [N, T] 或 [N] 的因子张量。
        prev_positions: 保留参数，连续模式下忽略。
        min_abs:        无信号阈值覆盖；None = Config 默认。
    """
...
def compute_target_positions_stateless(
    ...
    return compute_target_positions(factors, prev_positions=None, min_abs=min_abs)

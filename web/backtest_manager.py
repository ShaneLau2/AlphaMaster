...
class BacktestManager:
    ...
    def start(
        ...
        with self._lock:
            ...
            cmd.extend(["--hold-policy", hold_policy])
            if signal_threshold is not None:
                cmd.extend(["--threshold", str(signal_threshold)])
            if window_bars:
                cmd.extend(["--window-bars", str(int(window_bars))])
            cmd.extend(["--max-position-pct", str(min(200.0, max(1.0, float(max_position_pct))))])

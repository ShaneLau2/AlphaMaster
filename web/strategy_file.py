...
def timeframe_strategy_path(symbol: str, timeframe: str) -> Path:
    """timeframe 限定制品路径（如 best_BTCUSDT_H1.json）。

    同一交易对不同周期的冠军是不同制品（2026-09-06 起部署按
    symbol+timeframe 命名），解析默认策略时必须按数据文件周期匹配。
    """
    return STRATEGIES_DIR / f"best_{symbol}_{timeframe}.json"


def _same_file(a: str, b: str) -> bool:
    """两个路径是否指向同一文件（容忍相对/绝对、符号链接差异）。"""
    try:
...
def resolve_strategy_file(
    ...
    """默认策略解析优先级（生产接线，2026-09-06）：

    1. 显式已保存路径（settings.last_strategy_file，用户/UI 明选）
    2. 与所选数据文件 timeframe 匹配的 best_{symbol}_{tf}.json
       （如 H1 数据 → best_BTCUSDT_H1.json，即冻结的 H1 生产冠军）
    3. 旧式 best_{symbol}.json（仅当无 tf 匹配或未给 timeframe 时；
       保持 M5 时代制品可回退/可选）
    """
    if saved_path:
        p = Path(saved_path)
    ...
    if train_symbol:
        if timeframe:
            tf_path = timeframe_strategy_path(train_symbol, timeframe)
            if tf_path.exists():
                return str(tf_path.resolve())
...
def sync_best_strategy_for_symbol(
    ...
    *,
    ...
    if strat:
        ...
        if strat.get("data_source"):
            payload["data_source"] = strat["data_source"]
        # 溯源保护：train_range 一旦存在就必须随写随带（被覆盖会让回滚校验/
        # OOS 溯源退化为推断口径）。strat 有就用 strat 的；否则保留旧部署文件里的。
        tr = strat.get("train_range")
        if not tr and out_path.exists():
            try:
                tr = json.loads(out_path.read_text(encoding="utf-8")).get("train_range")
            except (OSError, ValueError):
                tr = None
        if tr:
            payload["train_range"] = tr
    payload = _apply_data_file_fallback(payload, data_file_hint=data_file_hint)
    # data_source 溯源与最终 data_file 指向同一文件才保留，
    # 防止 fallback 落到别的 parquet 时携带错误溯源
    ds = payload.get("data_source") or {}
    if (ds.get("data_file") and payload.get("data_file")
            and not _same_file(str(ds["data_file"]), str(payload["data_file"]))):
